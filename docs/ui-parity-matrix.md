...
| Missing | Microphone comparison recorder/player | Official records a resumable ten-second draft, then loops it through the live microphone chain so DSP edits can be heard without speaking again. StudioBridge currently draws a fixed timer, record dot, and play glyph as inert `Text` nodes with no callbacks or accessible names. Implement the evidence-backed local state machine below; never acquire a hardware-write lease or change routing automatically. | P1 |
...
| Partial | Global settings with no runtime effect | `beta_opt_in`, `mixing_suite_enabled`, and `meter_crossfade` remain persisted for compatibility, but now render as non-interactive controls with a visible unavailable reason and accessible disabled description. Implement each behavior before re-enabling it; profile crossfade remains the most user-visible gap. | P2 |
...
| Mic Output | Live meter with Peak/Talk zones, output gain, and a ten-second resumable recorder with continuously looping playback through the current microphone chain. | Live meter and read-only output gain are present. The comparison row is a static `"10.0s / 10.0s"`, red `●`, and green `▶`; none is interactive. | Keep output gain read-only. Implement the local recorder/player state machine below without device writes, guessed routes, or hidden monitoring changes. | Partial / P1 |
...
Implementation states should be explicit rather than inferred from timer text:
`Unavailable`, `Empty`, `Ready { clip }`, `Recording { prior_clip, draft,
remaining }`, `RecordingPaused { prior_clip, draft, remaining }`, and `Playing {
clip, elapsed }`. Audio work belongs on a bounded non-UI worker; UI callbacks send
commands, and worker events are the sole authority for timer and terminal-state
updates. Unit-test every transition above plus zero-length capture, stream error,
rapid double-click, navigation, shutdown, and device-loss cleanup.
...
1. Implement the still-inert recorder/player and copy-output surfaces only from
   captured behavior and safe audio-graph semantics. Beta, mixing-suite, and
   crossfade controls now state that they are unavailable and must remain
   disabled until their runtime consumers exist.
