...
if [[ "$(uname -s)" != "Linux" ]]; then
  printf 'This bootstrap must run inside the Fedora Live session.\n' >&2
  exit 1
fi
...
if [[ "$os_id" != "fedora" ]]; then
  printf 'Expected Fedora, detected %s. No packages were installed.\n' "$os_id" >&2
  exit 1
fi

cat <<'MESSAGE'
StudioBridge Fedora Live bootstrap
...
It does not install Fedora, modify partitions, start StudioBridge, install
...
cd "$workspace"
{
  printf '===== STUDIOBRIDGE FEDORA LIVE REPORT =====\n'
...
Open this ChatGPT conversation in Firefox and paste the report with:
  cat "$report"
